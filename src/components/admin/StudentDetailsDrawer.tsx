      ...
      <DrawerContent className="max-w-2xl">
            ...
            <div className="flex items-center gap-4">
              <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-slate-100">
                {data && data.photoUrl ? (
                  ...
                  <img src={data.photoUrl} alt="avatar" className="h-16 w-16 object-cover" />
                ) : (
        ...
        <div className="p-4 space-y-4 max-h-[70vh] overflow-auto">
          {loading ? (
            ...
            <div className="space-y-4">
              <Card>
                <h4 className="text-sm font-semibold text-slate-700 mb-2">Academic Info</h4>
                <div className="grid grid-cols-2 gap-4 text-sm text-slate-800">
                  ...
                  <div>
                    ...
                    <div className="mt-1 font-medium text-slate-900">{renderOrDash(data.classId)}</div>
                  </div>

                  <div>
                    <div className="text-xs text-slate-500">Roll No</div>
                    <div className="mt-1 font-medium text-slate-900">{renderOrDash(data.rollNo)}</div>
                  </div>
                  <div>
              ...
              <Card>
                <h4 className="text-sm font-semibold text-slate-700 mb-2">Personal Info</h4>
                <div className="grid grid-cols-2 gap-4 text-sm text-slate-800">
                  <div>
                    <div className="text-xs text-slate-500">Date of Birth</div>
                    <div className="mt-1">{data.profile?.dob ? new Date(data.profile.dob).toLocaleDateString() : "-"}</div>
                  </div>

                  <div>
                    <div className="text-xs text-slate-500">Country</div>
                    <div className="mt-1">{renderOrDash(data.profile?.country)}</div>
                  </div>

                  <div className="col-span-2">
                    <div className="text-xs text-slate-500">Address</div>
                    <div className="mt-1 text-sm text-slate-800">
                      {data.profile?.addressLine1 ? data.profile.addressLine1 : ""}
                      {data.profile?.addressLine2 ? `, ${data.profile.addressLine2}` : ""}
                      {!(data.profile?.addressLine1 || data.profile?.addressLine2) ? "-" : null}
                    </div>
                  </div>

                  <div>
                    <div className="text-xs text-slate-500">City</div>
                    <div className="mt-1">{renderOrDash(data.profile?.city)}</div>
                  </div>

                  <div>
                    <div className="text-xs text-slate-500">State / Postal</div>
                    <div className="mt-1">{renderOrDash(data.profile?.state)} {data.profile?.postalCode ? `, ${data.profile.postalCode}` : ""}</div>
                  </div>
                </div>
              </Card>

              <Card>
                <h4 className="text-sm font-semibold text-slate-700 mb-2">Guardian Details</h4>
                <div className="grid grid-cols-2 gap-4 text-sm text-slate-800">
                  <div>
                    <div className="text-xs text-slate-500">Primary Guardian</div>
                    <div className="mt-1">{renderOrDash(data.profile?.primaryGuardianName)}</div>
                    <div className="text-sm text-slate-600">{renderOrDash(data.profile?.primaryGuardianPhone)}</div>
                  </div>

                  <div>
                    <div className="text-xs text-slate-500">Secondary Guardian</div>
                    <div className="mt-1">{renderOrDash(data.profile?.secondaryGuardianName)}</div>
                    <div className="text-sm text-slate-600">{renderOrDash(data.profile?.secondaryGuardianPhone)}</div>
                  </div>

                  <div className="col-span-2">
                    <div className="text-xs text-slate-500">Emergency Contact</div>
                    <div className="mt-1">{renderOrDash(data.profile?.emergencyContact)}</div>
                  </div>
                </div>
              </Card>

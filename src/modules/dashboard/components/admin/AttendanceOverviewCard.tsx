...
import { fetchClasses } from "@/modules/classes";
import {
  Select,
  SelectTrigger,
  SelectValue,
...
import {
  postAttendanceGraph,
  type AttendanceGraphDataPoint,
} from "@/modules/dashboard/api/adminDashboard";
  ...
  React.useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const dataRes = await postAttendanceGraph(payload);
        if (!mounted) return;

        if (Array.isArray(dataRes)) {
          setChartData(dataRes as AttendanceGraphDataPoint[]);
